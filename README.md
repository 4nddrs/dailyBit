...
- Hovering a task reveals "Question", "Note", and "Task" buttons; a task can have several of each.
...
- A task's "Task" button assigns work with that task as `relatedTask` (its description, plus the origin report/section/task ids); on the day that task's own report shows, it renders right under the task — same row as the trailing "Assigned by lead" block, including its daily update for that developer/date — instead of in that trailing block, so it stays visually tied to the work it's about. On any other day, if the task was removed, or for an assignment made any other way (no origin task, or "Only my questions & tasks" mode, which hides task cards entirely), it shows in the trailing block as before; nothing is ever dropped, only relocated when there's a task to relocate it under.
- The daily update shown for an assignment (in the trailing "Assigned by lead" block, under its task, and in the developer's own "Preview as lead") is the assignee's update for the selected date if they wrote one that day, otherwise their most recent update from an earlier day (never one from a later day) — so a task the developer updated yesterday doesn't show "No updates" today just because the update itself is dated yesterday. When the shown update isn't from the selected date, a small "Last update · {its date}" label appears above it. "No updates" only when the assignee has never written one on or before the selected date. The developer's own per-day update editor is unaffected: it always shows and edits that exact date's own update, never a fallback from an earlier day.
- A developer's lead questions from earlier days — report-level or task-anchored — show under their card too, labeled "Asked on {origin date}" (task-anchored also shows "About task: {origin task description} — ") — even on a date the developer has no report for, alongside their assignments — and stay editable/removable there, targeting the day the question was originally asked. Unlike Developer View (which drops a question the day after it's answered), the lead's own view keeps an *answered* question visible, with its answer, through the next business day after it was answered (a Friday answer stays visible through Monday; Saturday/Sunday answers stay visible through Monday too), so the lead doesn't lose sight of it over a weekend. They count toward the "Only my questions & tasks" filter the same as today's lead questions.
...
| `assignments/{assignmentId}` | A lead-created task assigned to one or more developers: `{ description, assigneeIds, createdBy, startDate, status: 'open' | 'closed', closedDate?, relatedTask?, createdAt, updatedAt }`. Visible on a date when `startDate <= date` and the assignment is still `'open'` or `closedDate >= date`. Independent of `reports`. `relatedTask?: { description, reportId?, sectionId?, taskId? }` is set when created from the "Task" action on a developer's task (only `description` for one created any other way, or an older assignment); Lead View uses the ids to render it under that task instead of the trailing "Assigned by lead" block, on the day that task's report is `reportId` and the task still exists — otherwise it falls back to the trailing block. |
| `assignments/{assignmentId}/updates/{assigneeId}_{date}` | One assignee's daily answer to an assignment: `{ assigneeId, date, text?, links, createdAt, updatedAt }`. The developer's own editor always reads/writes the exact date's own doc; every lead-facing display instead shows the doc for the selected date if it exists, else the assignee's most recent doc before it (never one after), labeled with its date when it isn't the selected one — see `subscribeAssignmentUpdatesForDate`/`subscribeLatestAssignmentUpdate` in `src/services/firestore.ts`. |
...
- For `answer` specifically, the relevance check also judges whether a related answer is complete or only partial, and the response always carries a usable suggestion (never invented facts — an unknown fact is a `[bracketed placeholder]` the developer must fill in): a `200` with `{ suggestion, status: 'ok' }` for a complete answer (the normal rewrite, unchanged), `{ suggestion, status: 'partial' }` for a related-but-incomplete one (a better answer that keeps everything the developer wrote and adds placeholders for what's missing — shown as "Suggested better answer" with a reminder to fill in the placeholders before saving), or the `422` above with an added `exampleAnswer` (an example of what a good answer could look like) for an off-topic one — shown as an "Example answer" box the developer can use as a starting point, never applied automatically. An off-topic `option` likewise gets an `exampleAnswer` on its `422` (an example answer option, shown as an "Example option" box); a relevant option still returns just `{ suggestion }`, and `task`/`question` are unaffected.
